...
function get_city_name(array $components) {
...
function get_state_name(array $components) {
    foreach ($components as $component) {
...
function get_country_name(array $components) {
    foreach ($components as $component) {
...
function get_postal_code(array $components) {
    foreach ($components as $component) {
...
function get_political_address(array $components) {
    $city_name = null;
    $state_name = null;
    foreach ($components as $component) {
        if (in_array("administrative_area_level_2", $component->types)) {
...
function get_location(object $geometry) {
    if (property_exists($geometry, "geometry")) {
        $geo = $geometry->geometry->location;
        return \App\Models\LatLng::unserialize((array) $geo);
    }
    return null;
}
