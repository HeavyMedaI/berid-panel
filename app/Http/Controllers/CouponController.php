...
class CouponController extends Controller
{
    ...
    public function __construct(CouponRepository $couponRepo, CustomFieldRepository $customFieldRepo, UserRepository $userRepo, DiscountableRepository $discountableRepository)
    {
        parent::__construct();
    ...
    public function create()
    {
        ...
        return view('coupons.create')->with("customFields", isset($html) ? $html : false)
            #->with("eService", $eService)->with("eProvider", $eProvider)->with("category", $category)
            ->with("user", $user)
            #->with("eServicesSelected", $eServicesSelected)->with("eProvidersSelected", $eProvidersSelected)->with("categoriesSelected", $categoriesSelected)
    ...
    private function initDiscountables(array $input): array
    {
        ...
        if (isset($input['users'])) {
            foreach ($input['users'] as $userId) {
                $discountables[] = ["discountable_type" => "App\Models\User", "discountable_id" => $userId];
            }
        }
    ...
    public function edit(int $id)
    {
        ...
        $usersSelected = $coupon->discountables()->where("discountable_type", "App\Models\User")->pluck('discountable_id');
        ...
        return view('coupons.edit')->with('coupon', $coupon)->with("customFields", isset($html) ? $html : false)
            #->with("eService", $eService)->with("eProvider", $eProvider)->with("category", $category)
            ->with("user", $user)
            #->with("eServicesSelected", $eServicesSelected)->with("eProvidersSelected", $eProvidersSelected)->with("categoriesSelected", $categoriesSelected)
            ->with("usersSelected", $usersSelected);
